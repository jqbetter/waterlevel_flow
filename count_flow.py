...
class Radar(object):
    ...
    def q_data_filtering(self):#,requirements_dir=None):
        ...
        min_level = level[min_flow_date_local]
        ...
        o_date = [datetime.datetime.strptime(x, "%Y/%m/%d %H:%M") for x in date]
    ...
    def sec_data_filtering(self,datetime_format = r'%Y/%m/%d %H:%M'):
        #根据信噪比筛选数据
        collect_date_str = self.dis_information.keys()

        collect_date_datetime = [datetime.datetime.strptime(x,datetime_format) for x in collect_date_str]
        ...
        for idx_datetime in collect_date_str:
            x = datetime.datetime.strptime(idx_datetime, datetime_format)
            date_type.append(x)
    ...
    def calculate_q_when_z_unconfidence(self):
        ...
        dis_time = np.array([datetime.datetime.strptime(k, "%Y/%m/%d %H:%M") for k in dis_datetime])
        ...
        for idx in iters:
            collect_time = datetime.datetime.strptime(date_time_[idx],"%Y/%m/%d %H:%M")
            ...
            # dest_time = None
            # for k in dis_datetime:
            #     dis_time = datetime.datetime.strptime(k,"%Y/%m/%d %H:%M")
            #
            #     sub_time = collect_time - dis_time
            #     if sub_time >= -flag_time and sub_time <= flag_time:
            #         dest_time = k
            #         break
            #
            # if dest_time != None:
            #     dis_data = self.dis_information[dest_time]
            #     save_datetime.append(date_time_[idx])
            #     save_level.append(collect_level)
            #
            #     #local_high = []
            #     high = collect_level - self.section_high
            #     #print("水位差:",high)
            #     local_high = np.where(high>=0)[0]
            #     #local_high = np.delete(local_high,-1)
            #     # print(lls)
            #     # #print(self.section_dist)
            #     # # s_d = []
            #     # for i in range(self.len_section):
            #     #     if high[i] > 0:
            #     #         local_high.append(i)
            #             # s_d.append(self.section_dist[i])
            #     # print("local:",local_high)
            #     # print("分段距离：",s_d)
            #     #num_data = len(local_high)
            #
            #     water_flows = []
            #     river_areas = []
            #     water_v = []
            #     # min_x = np.min(local_high)
            #     # max_x = np.max(local_high)
            #     # print(min_x,max_x)
            #     #sss = []
            #     for i in local_high:
            #         # s1 = (self.section_dist[i+1]-self.section_dist[i])*(high[i]+high[i+1])*0.5
            #         # sss.append(s1)
            #         # print(s1)
            #         if i >= self.len_section:
            #             break
            #         if i == local_high[-1] and i < self.len_section:
            #             local_point = (self.section_dist[i] / 10).__round__(0)
            #             local_point = int(local_point)-1
            #             # for dis_tuple in dis_data:
            #             #     if abs(self.section_dist[i] - dis_tuple[0]) < 5:
            #             w1 = high[i]
            ...
            #         local_point = (current_distance/10).__round__(0)
            #         #print(local_point)
            #         local_point = int(local_point)-1
            ...
            #         #print(local_point)
            #         #for dis_tuple in dis_data:
            #             #if abs(current_distance - dis_tuple[0])<5:
            #         w1 = high[i] + high[i+1]
            #         h1 = self.section_dist[i+1]-self.section_dist[i]
            #         s = w1*h1*0.5
            #         river_areas.append(s)
            #         #print(dis_data[local_point])
            #         water_f =  s * dis_data[local_point][1]
            #         water_v.append(dis_data[local_point][1])
    ...
    def calculation_q_by_sec_velocity(self,datetime_str,level,keys=None,flag_time=None,dis_time=None,sec=10.0):
        ...
        idx = np.where(abs(datetime_str - dis_time) < flag_time)[0][0]
